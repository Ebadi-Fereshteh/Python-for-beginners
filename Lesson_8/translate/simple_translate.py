...
def En2fa():
    ...
    # second method > convert list to str
    str = " "
    tr_txt = str.join(translate_fa)
    text2voice(tr_txt)
    ...
    for i in range(len(stm_split)):
        for j in range(len(dictionary)):
            if(stm_split[i]== dictionary[j]['persian']):
                ...
                break            
    # one method > convert list to str
    print(*translate_en, sep=' ')

    # second method > convert list to str
    str = " "
    tr_txt = str.join(translate_en)
    text2voice(tr_txt)
    result = input('try again? (y/n): ')
    if result== 'y':
        fa2En()
def text2voice(tr_text):
    tr_voice = gtts.gTTS(tr_text, lang="en", slow=False)
    tr_voice.save('translate/voice.mp3')
    play_voice()

def play_voice():
    p = vlc.MediaPlayer('translate/voice.mp3')
    p.play()
