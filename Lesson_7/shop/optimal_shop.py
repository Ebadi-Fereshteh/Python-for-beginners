...
def load(flag):
    ...
    # read data from file
    mydata = open('shop/database.txt','r')
    # data = mydata.read()

    # split each line from file
    product_list = mydata.read().split('\n')
    # print(product_list)
...
def search_product():
    print('\n -----------------------------  search product --------------------------------')
    key = input('Enter id or the name of product or part of it: ')
    flag = True
    for i in range(len(PRODUCTS)):
        if key in PRODUCTS[i]['name'] or key == str(PRODUCTS[i]['id']):
...
def print_factor():
    ...
    for i in range(len(user_cart)):
        total = (float(user_cart[i]['count']) * float(user_cart[i]['price']))
        total = total + sum
    ...
    for i in range(len(PRODUCTS)):
        if PRODUCTS[i]['id']== str(id):            
            ...
            if num <= int(PRODUCTS[i]['count']):
                ...
                id_duplicate = 0
                for j in range(len(user_cart)):
                    if user_cart[j]['id'] == PRODUCTS[i]['id']:
                        id_duplicate = j
                        break
                if id_duplicate== 0:
                    ...
                    user_cart[id_duplicate]['count'] = user_cart[id_duplicate]['count'] + num 
...
def save_exit():
    ...
    # print(data)
    for i in range(len(PRODUCTS)):
        row = PRODUCTS[i]['id']+ ','+ PRODUCTS[i]['name']+ ','+ PRODUCTS[i]['price']+ ',' + PRODUCTS[i]['count']+'\n'
        print(row)
